...
import { getCreatureName } from "@/constants/creatures";
...
export default function GameScreen() {
  ...
  const loadGameState = async () => {
    const savedHigh = await AsyncStorage.getItem("highScore");
    if (savedHigh) {
      setHighScore(parseInt(savedHigh, 10));
    }

    const savedCreatures = await AsyncStorage.getItem("unlockedCreatures");
    if (savedCreatures) {
      creatureCacheRef.current = new Set(JSON.parse(savedCreatures));
    } else {
      // Initialize with common creatures
      creatureCacheRef.current = new Set(["butterfly", "bird", "firefly"]);
    }
  };
  ...
  const handleGameOver = useCallback(
    async (finalScore: number, creatures: string[]) => {
      ...
      // Save unlocked creatures
      const allUnlocked = new Set(creatureCacheRef.current);
      creatures.forEach((c) => allUnlocked.add(c));
      await AsyncStorage.setItem(
        "unlockedCreatures",
        JSON.stringify(Array.from(allUnlocked))
      );
      creatureCacheRef.current = allUnlocked;
  ...
  const getCreatureDisplayName = (creatureId: string): string => {
    return getCreatureName(creatureId);
  };
          ...
          <View className="bg-white rounded-3xl p-6 w-4/5 max-w-sm shadow-2xl">
            ...
            {newCreatures.length > 0 && (
              <View className="bg-blue-50 rounded-2xl p-4 mb-4">
                ...
                {newCreatures.map((creatureId) => (
                  <Text
                    key={creatureId}
                    className="text-center text-base font-semibold text-blue-700 mb-1"
                  >
                    {getCreatureDisplayName(creatureId)}
                  </Text>
                ))}
                <Text className="text-center text-3xl mt-2">
                  {newCreatures.map((c) => {
            ...
            <View className="gap-3">
              <TouchableOpacity
                onPress={handleRetry}
                className="bg-gradient-to-r from-teal-400 to-teal-500 rounded-full py-3 active:opacity-80"
              >
              ...
              <TouchableOpacity
                onPress={handleHome}
                className="bg-gradient-to-r from-purple-400 to-purple-500 rounded-full py-3 active:opacity-80"
              >
                <Text className="text-center text-white font-bold text-lg">
                  🏠 Home
                </Text>
